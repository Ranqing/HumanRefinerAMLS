...
def qing_rewrite_dsp(workdir, imgname, dspname):
    ...
    print(f_dspname)
    print(f_imgname)
    dspdatas = qing_read_2d_txt(f_dspname)
    dspdatas_1d = []
    rows = len(dspdatas)
    for r in range(rows):
        cols = len(dspdatas[r])
...
def qing_read_stereo_info(infoname):
    st_x_0 = 0
    st_y_0 = 0
    st_x_1 = 0
    st_y_1 = 0
    stereo_mtx = np.zeros((4, 4))
    with open(infoname, 'r') as f:
        ...
        for line in data:
            ...
            if cnt == 10:
                odom = line.split()
                numbers_int = list(map(int, odom))
                st_x_0 = numbers_int[0]
                st_y_0 = numbers_int[1]
            elif cnt == 11:
                odom = line.split()
                numbers_int = list(map(int, odom))
                st_x_1 = numbers_int[0]
                st_y_1 = numbers_int[1]
            elif cnt >= 15:
                odom = line.split()
                numbers_float = list(map(float, odom))
                stereo_mtx[cnt - 15, 0] = numbers_float[0]
                stereo_mtx[cnt - 15, 1] = numbers_float[1]
                stereo_mtx[cnt - 15, 2] = numbers_float[2]
                stereo_mtx[cnt - 15, 3] = numbers_float[3]
    return st_x_0, st_y_0, (st_x_0 - st_x_1), stereo_mtx

# refine disparity using 1d_mls along x, y direction sequentially


def adaptive_mls_1d(workdir, dspname, mskname, dsptxt):
    dspmtx = cv2.imread(workdir + dspname, 0)
...
# def adaptive_mls_2d(workdir, dspname, mskname, dsptxt):
#     dspmtx = cv2.imread(workdir + dspname, 0)
#     mskmtx = cv2.imread(workdir + mskname, 0)
#     ret, thresh_msk = cv2.threshold(mskmtx, 75, 255, cv2.THRESH_BINARY)
#     height, width = dspmtx.shape
...
def main():
    ...
    # workdir = './test_new/'
    # imgname = 'crop_imgL.jpg'        # 550x950
    # mskname = 'crop_mskL.jpg'
    # dspname = 'final_disp_l_0.jpg'
    ...
    imgmtx = cv2.imread(workdir + imgname, 1)
    print(type(imgmtx), imgmtx.shape)

    # about calibration data
    # st_x, st_y, base_d, stereo_mtx = qing_read_stereo_info(
    #     workdir + stereoinfo)
    st_x, st_y, base_d, stereo_mtx = qing_read_stereo_info(
        workdir + stereoinfo)
    ...
    # print(dsp_name)
    dsp_name = workdir + 'rewrite_final_disp_l_2.txt'
    ...
    # output_dsp_txt, dsp_data = adaptive_mls_2d(
    #     workdir, dspname, mskname, dsptxt)

    # dsp_data = np.loadtxt('./output/mls_disp_y_after_x.txt')
    # print('dsp_data: ', dsp_data.shape)
    ply_name = workdir + 'mls_A01A02.ply'
